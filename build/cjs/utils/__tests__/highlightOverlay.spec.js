...
var _highlightOverlay = require("../highlightOverlay.js");
...
const createMockMapgl = ({
  ...
  return {
    ...
    getSource: jest.fn(id => id === existingSourceId ? source : undefined),
...
describe('highlightOverlay', () => {
  it('clones eligible layers, halos icon layers, and skips fill/label layers', () => {
    ...
    const overlayIds = (0, _highlightOverlay.createHighlightOverlay)(map, {
      id: 'layer-1',
      glLayers: [{
        id: 'layer-1-point',
        type: 'circle',
    ...
    expect(mapgl.addSource).toHaveBeenCalledWith((0, _highlightOverlay.getOverlaySourceId)('layer-1'), expect.objectContaining({
      type: 'geojson'
    }));
    expect(overlayIds).toEqual(['layer-1-point-highlight', 'layer-1-outline-highlight', 'layer-1-symbol-highlight-halo', 'layer-1-symbol-highlight-icon']);
    expect(mapgl.addLayer).toHaveBeenCalledTimes(4);
    expect(mapgl.addLayer).toHaveBeenCalledWith(expect.objectContaining({
      id: 'layer-1-point-highlight',
      type: 'circle',
      source: (0, _highlightOverlay.getOverlaySourceId)('layer-1')
    }), 'before-id');
  });
  it('adds a colored halo behind the icon clone for icon layers', () => {
    const mapgl = createMockMapgl();
    const map = createMockMap(mapgl);
    const isSymbolFilter = ['all', ['==', '$type', 'Point'], ['has', 'iconUrl']];
    const overlayIds = (0, _highlightOverlay.createHighlightOverlay)(map, {
      id: 'layer-1',
      glLayers: [{
        id: 'layer-1-symbol',
    ...
    expect(overlayIds).toEqual(['layer-1-symbol-highlight-halo', 'layer-1-symbol-highlight-icon']);
    // Halo added first, so the icon clone renders on top of it.
    expect(mapgl.addLayer).toHaveBeenNthCalledWith(1, expect.objectContaining({
    ...
    expect(mapgl.addLayer).toHaveBeenNthCalledWith(2, expect.objectContaining({
  ...
  it('omits the filter when the base layer has none, rather than matching everything', () => {
    const mapgl = createMockMapgl();
    const map = createMockMap(mapgl);
    (0, _highlightOverlay.createHighlightOverlay)(map, {
      id: 'layer-1',
      glLayers: [{
        id: 'layer-1-symbol',
        type: 'symbol',
  ...
  it('does not re-add a source/layer that already exists', () => {
    const mapgl = createMockMapgl({
      existingSourceId: (0, _highlightOverlay.getOverlaySourceId)('layer-1'),
  ...
  it('updates the overlay source data and marks features hover+selected with the given color', () => {
    ...
    const map = createMockMap(mapgl);
    const source = mapgl.getSource((0, _highlightOverlay.getOverlaySourceId)('layer-1'));
    const feature = {
      id: 7,
      type: 'Feature',
      properties: {}
    ...
    expect(source.setData).toHaveBeenCalledWith({
      ...
      features: [_objectSpread(_objectSpread({}, feature), {}, {
        id: 1
      })]
    });
  ...
  it('re-keys features with source-local ids to avoid cross-sub-layer id collisions', () => {
    const mapgl = createMockMapgl({
      existingSourceId: (0, _highlightOverlay.getOverlaySourceId)('group-1')
    });
    const map = createMockMap(mapgl);
    ...
    const source = mapgl.getSource((0, _highlightOverlay.getOverlaySourceId)('group-1'));
  ...
  it('no-ops when the overlay source does not exist yet', () => {
    const mapgl = createMockMapgl();
    const map = createMockMap(mapgl);
    expect(() => (0, _highlightOverlay.updateHighlightOverlay)(map, {
      id: 'missing',
      features: [],
      color: '#FFC800'
    })).not.toThrow();
  ...
  it('removes overlay layers and source', () => {
    ...
    const map = createMockMap(mapgl);
    (0, _highlightOverlay.removeHighlightOverlay)(map, 'layer-1', ['layer-1-point-highlight']);
    expect(mapgl.removeLayer).toHaveBeenCalledWith('layer-1-point-highlight');
    expect(mapgl.removeSource).toHaveBeenCalledWith((0, _highlightOverlay.getOverlaySourceId)('layer-1'));
  });
});