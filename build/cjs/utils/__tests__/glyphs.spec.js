...
const ownGlyphsUrl = 'https://example.com/dhis-web-maps/fonts/{fontstack}/{range}.pbf';
const ownGlyphUrl = (font, range = '0-255') => `https://example.com/dhis-web-maps/fonts/${font}/${range}.pbf`;
const otherGlyphUrl = (font, range = '0-255') => `https://tiles.openfreemap.org/fonts/${font}/${range}.pbf`;
describe('createFontTransformRequest', () => {
  ...
  it('does not touch other resource types', () => {
    const transformRequest = (0, _glyphs.createFontTransformRequest)(ownGlyphsUrl);
    expect(transformRequest(otherGlyphUrl('Open Sans Regular'), 'Tile')).toBeNull();
  });
  it('does not throw or redirect when no glyphs URL is configured', () => {
    const transformRequest = (0, _glyphs.createFontTransformRequest)(undefined);
    expect(() => transformRequest(otherGlyphUrl('Open Sans Regular'), 'Glyphs')).not.toThrow();
    expect(transformRequest(otherGlyphUrl('Open Sans Regular'), 'Glyphs')).toBeNull();
  });
});