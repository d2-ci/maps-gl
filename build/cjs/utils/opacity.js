...
exports.setLayersOpacity = exports.clearLayerOpacityCache = void 0;
const properties = {
...
const getSubLayerIds = (mapgl, id) => {
  ...
  if (!instanceCache.has(id)) {
    instanceCache.set(id, mapgl.getStyle().layers.filter(layer => layer.id.startsWith(id)).map(layer => layer.id));
  }
  return instanceCache.get(id);
};
const clearLayerOpacityCache = (mapgl, id) => {
  _subLayerCache.get(mapgl)?.delete(id);
};
exports.clearLayerOpacityCache = clearLayerOpacityCache;
