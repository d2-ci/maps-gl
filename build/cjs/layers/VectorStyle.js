...
class VectorStyle extends _maplibreGl.Evented {
  ...
  async removeFrom() {
    ...
    if (this._map.getMapGL()) {
      await this.toggleVectorStyle(false, (0, _style.mapStyle)({
        glyphs
      }));
    }
  }
  ...
  setStyle(style) {
    return new Promise((resolve, reject) => {
      ...
      const onError = e => {
        ...
        } else if (e.error.message.includes('r.blob is not a function')) {
  ...
  setOpacity(opacity) {
    ...
    if (mapgl && this.isOnMap() && this._styleLayers) {
      (0, _opacityVectorStyle.setVectorStyleOpacity)(mapgl, this._styleLayers, {
        opacity,
  ...
  setVisibility(isVisible) {
    if (this.isOnMap()) {
      ...
      this._visibleLayers.forEach(id => mapgl.setLayoutProperty(id, 'visibility', value));
    }
