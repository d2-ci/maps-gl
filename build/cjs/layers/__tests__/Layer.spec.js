...
describe('Layer', () => {
  ...
  it('Should re-move the highlight overlay after its base layers when moved, so it stays on top', () => {
    ...
    expect(mockMapGL.moveLayer).toHaveBeenCalledWith('layer-1-highlight', 'before-id');
    // Overlay must be moved after the base layer
    const movedIds = mockMapGL.moveLayer.mock.calls.map(call => call[0]);
    expect(movedIds.indexOf('layer-1-highlight')).toBeGreaterThan(movedIds.indexOf('layer-1'));
  });
  ...
  it('Should update the highlight overlay on highlight(), not the base source', () => {
    ...
    layer.addTo(mockMap);
    _highlightOverlay.updateHighlightOverlay.mockClear(); // addTo() flushes the (empty) overlay
    layer.highlight('fdc6uOvgoji');
    expect(_highlightOverlay.updateHighlightOverlay).toHaveBeenCalledTimes(1);
    expect(_highlightOverlay.updateHighlightOverlay.mock.calls[0][1].features).toMatchObject([{
      id: 2,
      source
    }]);
    layer.highlight(1);
    expect(_highlightOverlay.updateHighlightOverlay).toHaveBeenCalledTimes(2);
    expect(_highlightOverlay.updateHighlightOverlay.mock.calls[1][1].features).toMatchObject([{
      id: 1,
      source
    }]);
    layer.highlight('abc');
    expect(_highlightOverlay.updateHighlightOverlay).toHaveBeenCalledTimes(3);
    expect(_highlightOverlay.updateHighlightOverlay.mock.lastCall[1].features).toEqual([]);
  ...
  it('Should forward a caller-chosen color to highlight()/select()', () => {
    ...
    expect(_highlightOverlay.updateHighlightOverlay.mock.lastCall[1].color).toBe('#FFC800');
    layer.select(['O6uvpzGd5pu'], '#FF0000');
    expect(_highlightOverlay.updateHighlightOverlay.mock.lastCall[1].color).toBe('#FF0000');
  });
  it('Should combine hover and a persistent selection into one overlay update instead of one clobbering the other', () => {
    ...
    expect(_highlightOverlay.updateHighlightOverlay.mock.lastCall[1].features).toMatchObject([{
      id: 2,
      source
    }, {
      id: 1,
  ...
  it('Should let the last highlight()/select() color win for the whole overlay, by design', () => {
    ...
    expect(_highlightOverlay.updateHighlightOverlay.mock.lastCall[1].color).toBe('blue');
  });
  it('Should replay a pending highlight into the overlay once it is (re-)created, e.g. after a style reload', () => {
    ...
    layer.addTo(mockMap);
    expect(_highlightOverlay.updateHighlightOverlay).toHaveBeenCalledTimes(1);
    expect(_highlightOverlay.updateHighlightOverlay.mock.calls[0][1]).toMatchObject({
      features: [{
        id: 2,
        source
      }],
  ...
  it('Should restrict rendered features to the given ids, ANDed with any existing static filter', () => {
    ...
    expect(mockMapGL.setFilter).toHaveBeenCalledWith('layer-1', ['all', ['==', '$type', 'Polygon'], ['in', ['get', 'id'], ['literal', ['O6uvpzGd5pu', 'fdc6uOvgoji']]]]);
    expect(mockMapGL.setFilter).toHaveBeenCalledWith('layer-1-points', ['in', ['get', 'id'], ['literal', ['O6uvpzGd5pu', 'fdc6uOvgoji']]]);
  });
  ...
  it('Should hide/show the highlight overlay along with its base layer, so a hidden layer has no leftover glow', () => {
    ...
    layer.setVisibility(false);
    expect(mockMapGL.setLayoutProperty).toHaveBeenCalledWith('layer-1', 'visibility', 'none');
    expect(mockMapGL.setLayoutProperty).toHaveBeenCalledWith('layer-1-highlight', 'visibility', 'none');
  });
  ...
  it('Should apply opacity once the style is loaded', () => {
    ...
    expect(mockMapGL.setPaintProperty).toHaveBeenCalledWith(layerId, 'fill-opacity', 0.5);
  ...
  it('Should only style sub-layers that actually exist', () => {
    ...
    layer._overlayLayerIds = [];
    layer.setVisibility(false);
    expect(mockMapGL.setLayoutProperty).toHaveBeenCalledWith('x-polygon', 'visibility', 'none');
    expect(mockMapGL.setLayoutProperty).not.toHaveBeenCalledWith('x-point', 'visibility', expect.anything());
  });
  ...
  it('Should only move sub-layers that actually exist', () => {
    ...
    mockMapGL.getLayer.mockImplementation(id => id === 'x-polygon');
    layer.move();
    expect(mockMapGL.moveLayer).toHaveBeenCalledWith('x-polygon', 'before-id');
    expect(mockMapGL.moveLayer).not.toHaveBeenCalledWith('x-point', expect.anything());
  });
});