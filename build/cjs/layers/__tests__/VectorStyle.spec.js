...
describe('VectorStyle opacity', () => {
  ...
  it('applies a configured labelOpacity at load time, even at the default basemap opacity', async () => {
    ...
    expect(mapgl.setPaintProperty).toHaveBeenCalledWith('water', 'fill-opacity', 1);
  });
  ...
  it('does not touch layers added after the vector style loaded (e.g. overlays)', async () => {
    ...
    expect(mapgl.setPaintProperty).not.toHaveBeenCalledWith('thematic-polygon', 'fill-opacity', expect.anything());
  });
  ...
  it('restores full opacity', async () => {
    ...
    expect(mapgl.setPaintProperty).toHaveBeenCalledWith('water', 'fill-opacity', 0.8);
  });
  it('re-applies the current opacity to a newly loaded style, using its own base values', async () => {
    ...
    await vectorStyle.toggleVectorStyle(true, 'https://example.com/dark.json');
    expect(mapgl.setPaintProperty).toHaveBeenCalledWith('ocean', 'fill-opacity', 0.3);
  });
  ...
  it('suppresses a stale result across two different VectorStyle instances sharing a map', async () => {
    ...
    const map = createMap(mapgl);
    const oldBasemap = new _VectorStyle.default({
      url: 'https://example.com/a.json'
    });
    const newBasemap = new _VectorStyle.default({
      url: 'https://example.com/b.json'
    });
    oldBasemap._map = map;
    newBasemap._map = map;
    const removeCall = oldBasemap.toggleVectorStyle(false, 'https://example.com/default.json');
    await flushMicrotasks();
  ...
  it('settles an earlier pending load and clears the loading flag when a later call skips setStyle() entirely', async () => {
    ...
    const secondCall = vectorStyle.toggleVectorStyle(false, 'https://example.com/default.json');
    await flushMicrotasks();
    expect(map._styleIsLoading).toBe(false);
    await expect(firstCall).resolves.toBeUndefined();
    await expect(secondCall).resolves.toBeUndefined();
  ...
  it('waits for every overlay to finish being added before resolving', async () => {
    ...
    const overlay = {
      isOnMap: jest.fn(() => false),
      addTo: jest.fn(() => new Promise(resolve => resolveAdd = resolve)),
      removeFrom: jest.fn(async () => {}),
      setVisibility: jest.fn(),
      isVisible: jest.fn(() => true)
    };
    ...
    vectorStyle._map = map;
    let resolved = false;
    const addPromise = vectorStyle.addOtherLayers().then(() => resolved = true);
    await flushMicrotasks();
  ...
  it('waits for every overlay to finish being removed before resolving', async () => {
    ...
    const overlay = {
      ...
      removeFrom: jest.fn(() => new Promise(resolve => resolveRemove = resolve))
    };
    map.getLayers = jest.fn(() => [overlay]);
    const vectorStyle = new _VectorStyle.default({
      url: 'https://example.com/a.json'
    ...
    vectorStyle._map = map;
    let resolved = false;
    const removePromise = vectorStyle.removeOtherLayers().then(() => resolved = true);
    await flushMicrotasks();
    expect(resolved).toBe(false);
    resolveRemove();
    await removePromise;
