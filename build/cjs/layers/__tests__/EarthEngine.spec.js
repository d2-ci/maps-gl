...
function _toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); } /* global mockMap, mockMapGL */
...
describe('EarthEngine', () => {
  ...
  describe('raster mask (filter/setVisibleIds)', () => {
    const featureIds = f => f.features.map(({
      properties
    }) => properties.id);
    const addLayerWithMaskSource = async (layerOptions = options) => {
      const layer = new _EarthEngine.default(layerOptions);
      const maskSource = {
        setData: jest.fn()
      };
      mockMapGL.getSource.mockImplementation(id => id === `${layer.getId()}-mask` ? maskSource : {
        setData: jest.fn()
      });
      await layer.addTo(mockMap);
      return {
    ...
    it('filter() masks the features it excludes', async () => {
      const {
        layer,
        maskSource
      } = await addLayerWithMaskSource();
      layer.filter(['O6uvpzGd5pu']);
      expect(featureIds(maskSource.setData.mock.lastCall[0]).sort()).toEqual(['DiszpKrYNg8', 'fdc6uOvgoji'].sort());
    });
    ...
    it('masks the union of both exclusions when filter() and setVisibleIds() are both active', async () => {
      ...
      expect(featureIds(maskSource.setData.mock.lastCall[0]).sort()).toEqual(['DiszpKrYNg8', 'O6uvpzGd5pu'].sort());
    });
    ...
    it('does not narrow the mask layer itself via setVisibleIds, since its data is already the complement', async () => {
      ...
      layer.setVisibleIds(['O6uvpzGd5pu']);
      const maskFilterCalls = mockMapGL.setFilter.mock.calls.filter(([layerId]) => layerId === `${layer.getId()}-mask`);
