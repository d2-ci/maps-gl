...
describe('EarthEngine', () => {
  ...
  describe('raster mask (filter/setVisibleIds)', () => {
    ...
    it('filter() masks the features it excludes', async () => {
      ...
      const lastCall = maskSource.setData.mock.calls[maskSource.setData.mock.calls.length - 1][0];
      expect(featureIds(lastCall).sort()).toEqual(['DiszpKrYNg8', 'fdc6uOvgoji'].sort());
    });
    ...
    it('masks the union of both exclusions when filter() and setVisibleIds() are both active', async () => {
      ...
      const lastCall = maskSource.setData.mock.calls[maskSource.setData.mock.calls.length - 1][0];
      expect(featureIds(lastCall).sort()).toEqual(['DiszpKrYNg8', 'O6uvpzGd5pu'].sort());
    });
