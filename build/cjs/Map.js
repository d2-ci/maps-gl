...
class MapGL extends _maplibreGl.Evented {
  ...
  constructor(el, options = {}) {
    ...
    _defineProperty(this, "onMouseMove", evt => {
      ...
      if (feature) {
        ...
        if (layer) {
          ...
          hoverTarget = typeof layer.getSubLayerFromId === 'function' && layer.getSubLayerFromId(feature.layer.id) || layer;
        }
      } else {
        const target = evt && evt.originalEvent && evt.originalEvent.target;
        if (!target || !target.closest || !OVERLAY_SELECTORS.some(sel => target.closest(sel))) {
          this.hideLabel();
        }
      }
      const featureId = feature?.properties?.id ?? null;
      const nextHover = hoverTarget && featureId !== null ? hoverTarget : null;
      if (nextHover !== this._hoveredLayer || featureId !== this._hoveredFeatureId) {
        if (this._hoveredLayer) {
          this._hoveredLayer.fire('mouseleave');
        }
        if (nextHover) {
          nextHover.fire('mouseenter', {
            feature
          });
        }
    ...
    _defineProperty(this, "onMouseOut", evt => {
      const related = evt && evt.originalEvent && evt.originalEvent.relatedTarget;
      if (related && related.closest && OVERLAY_SELECTORS.some(sel => related.closest(sel))) {
        return;
      }
      this.hideLabel();
      if (this._hoveredLayer) {
  ...
  setHoverState(features, color) {
    const key = `${(0, _core.getFeaturesString)(features)}|${color ?? ''}`;
    if (key !== this._hoverStateKey) {
      ...
      if (Array.isArray(features)) {
        ...
        features.forEach(feature => this.setFeatureState(feature, color !== undefined ? {
          hover: true,
  ...
  setSelectedState(features, color) {
    ...
    if (key !== this._selectedStateKey) {
      ...
      if (this._selectedFeatures) {
        this._selectedFeatures.forEach(feature => this.setFeatureState(feature, {
          selected: false
        }));
        this._selectedFeatures = null;
      }
  ...
  getEventFeature(evt) {
    if (!this._interactiveLayerIds) {
      this._interactiveLayerIds = this.getLayers().filter(l => l.isInteractive()).flatMap(l => l.getInteractiveIds());
    }
    return this._interactiveLayerIds.length ? this._mapgl.queryRenderedFeatures(evt.point, {
  ...
  showLabel(content, lnglat, {
    ...
    const label = isHTML ? this._label.setHTML(content) : this._label.setText(content);
