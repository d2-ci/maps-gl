...
const makeImage = () => {
  const img = {};
  img.select = jest.fn(function () {
    if (arguments.length && typeof (arguments.length <= 0 ? undefined : arguments[0]) === 'number') {
      return {
        projection: jest.fn(() => ({
...
const createReducerInstance = () => {
  ...
  instance.combine = function () {
    ee.Reducer.combine(...arguments);
    return instance;
  };
  return instance;
};
...
imageCollectionObj.first = jest.fn(() => ({
  bandNames: bandNamesMock()
}));
...
imageCollectionObj.size = jest.fn(() => ({
  gt: jest.fn(() => true)
}));
...
ee.Algorithms = {
  If: jest.fn((cond, a, b) => cond ? a : b)
};
ee.List = {
  sequence: jest.fn(() => [0, 1, 2])
};
ee.Number = jest.fn(n => {
