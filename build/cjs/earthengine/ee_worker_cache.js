...
_defineProperty(WorkerCache, "flushExpired", (ttl = DEFAULT_TTL_MS) => {
  ;
  (async () => {
    try {
      const db = await openDB();
      ...
      cursorRequest.onerror = event => {
        console.error('Failed to iterate over entries while flushing ee_worker cache', event.target.error);
      };
      await new Promise(resolve => {
        tx.oncomplete = () => resolve();
        tx.onerror = () => {
          console.error('Transaction error while flushing ee_worker cache', tx.error);
          resolve();
        };
        tx.onabort = () => {
          console.error('Transaction aborted while flushing ee_worker cache', tx.error);
          resolve();
        };
      });
    } catch (error) {
