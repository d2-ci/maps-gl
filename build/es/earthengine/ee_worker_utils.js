...
export const filterCollectionByDateRange = (collection, startDate, endDate) => {
  return collection.filter(ee.Filter.or(ee.Filter.date(ee.Date(startDate.getTime()), ee.Date(endDate.getTime())), ee.Filter.and(ee.Filter.lt('system:time_start', endDate.getTime()), ee.Filter.gt('system:time_end', startDate.getTime()))));
};
...
export const getAdjustedScale = (features, eeScale) => {
  ...
  const minArea = ee.Number(featuresWithArea.reduceColumns({
    reducer: ee.Reducer.min(),
    selectors: ['area']
  }).get('min'));
  return ee.Algorithms.If(minArea.lt(cappedScale.pow(2)), minArea.sqrt().divide(2), cappedScale);
};
...
const computeMinMaxAndAlign = ({
  ...
  const minStartRaw = collection.reduceColumns(ee.Reducer.min(), ['system:time_start']).get('min');
  const maxStartRaw = collection.reduceColumns(ee.Reducer.max(), ['system:time_start']).get('max');
  const maxEndRaw = collection.reduceColumns(ee.Reducer.max(), ['system:time_end']).get('max');
  let minDate = overrideDate ? ee.Date(overrideDate.getTime()) : ee.Date(minStartRaw);
...
const buildStepsAndBandNames = ({
  ...
  const isJan1Thursday = ee.Number.parse(ee.Date.fromYMD(Number.parseInt(year), 1, 1).format('e')).eq(4);
...
export const aggregateTemporal = ({
  ...
  const temporalReducer = ee.Reducer[reducer] ? ee.Reducer[reducer]() : ee.Reducer.mean();
...
export const aggregateTemporalWeighted = ({
  ...
  const weightedImages = steps.map(s => {
    ...
    const withOverlap = collection.map(img => {
      ...
      const overlapStart = ee.Date(ee.Algorithms.If(imgStart.millis().gt(startDate.millis()), imgStart, startDate));
      const overlapEnd = ee.Date(ee.Algorithms.If(imgEnd.millis().lt(endDate.millis()), imgEnd, endDate));
  ...
  return ee.ImageCollection.fromImages(weightedImages).sort('system:time_start', false);
};
