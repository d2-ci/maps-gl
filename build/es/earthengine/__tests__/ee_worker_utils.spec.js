import { hasClasses, getHistogramStatistics, getStartOfEpiYear, getPeriodDates } from '../ee_worker_utils.js';
const scale = 1000;
const style = [{
  value: 9,
  name: 'Savannas',
...
describe('earthengine - getPeriodDates', () => {
  ...
  test('monthly reducer uses ee.Date.fromYMD (mocked) for start/end', () => {
    const {
      startDate,
      endDate
    } = getPeriodDates('EE_MONTHLY', 2015);
    expect(startDate instanceof Date).toBe(true);
    expect(endDate instanceof Date).toBe(true);
