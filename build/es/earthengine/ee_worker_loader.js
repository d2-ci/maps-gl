...
const getEarthEngineWorker = getAuthToken => new Promise((resolve, reject) => {
  if (resolvedWorker) {
    ...
    const EarthEngineWorker = wrap(typeof SharedWorker !== 'undefined' ? new SharedWorker(new URL('../earthengine/ee_worker.js', import.meta.url)).port : new Worker(new URL('../earthengine/ee_worker.js', import.meta.url)));
