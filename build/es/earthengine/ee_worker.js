...
import { getInfo, getScale, hasClasses, combineReducers, getClassifiedImage, getHistogramStatistics, getFeatureCollectionProperties } from './ee_worker_utils';
...
class EarthEngineWorker {
  ...
  getImage() {
    ...
    if (!filter) {
      ...
      filter.forEach(f => {
        collection = collection.filter(ee.Filter[f.type].apply(this, f.arguments));
      });
  ...
  async getAggregations(config) {
    ...
    const useHistogram = singleAggregation && hasClasses(aggregationType) && legend;
