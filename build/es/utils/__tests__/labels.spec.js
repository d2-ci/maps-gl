...
import { circleRadius, labelColor, labelFontSize, textOpacity } from '../style.js';
const id = 'abc';
const opacity = 0.5;
const fontSize = 10;
...
describe('labels', () => {
  it('Handles different label sources scenario', () => {
    ...
    const received = labelSource(features, {
      fontSize,
      labelNoData
    }, isBoundary);
  ...
  it('Should default text-offset from circleRadius and labelFontSize', () => {
    ...
    expect(layer.layout['text-offset']).toEqual([0, circleRadius / labelFontSize + 0.4]);
  });
  ...
  it('Should default text-color to a labelColor expression', () => {
    ...
    expect(layer.paint['text-color']).toEqual(['case', ['has', 'color'], ['get', 'color'], labelColor]);
  });
