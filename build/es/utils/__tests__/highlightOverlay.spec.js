...
import { getOverlaySourceId, createHighlightOverlay, updateHighlightOverlay, removeHighlightOverlay } from '../highlightOverlay.js';
...
describe('highlightOverlay', () => {
  it('clones eligible layers, halos icon layers, and skips fill/label layers', () => {
    ...
    expect(mapgl.addSource).toHaveBeenCalledWith(getOverlaySourceId('layer-1'), expect.objectContaining({
      type: 'geojson'
    }));
  ...
  it('no-ops when the overlay source does not exist yet', () => {
    const mapgl = createMockMapgl();
    const map = createMockMap(mapgl);
    expect(() => updateHighlightOverlay(map, {
      id: 'missing',
      features: [],
      color: '#FFC800'
    })).not.toThrow();
  ...
  it('removes overlay layers and source', () => {
    ...
    expect(mapgl.removeSource).toHaveBeenCalledWith(getOverlaySourceId('layer-1'));
  });
});