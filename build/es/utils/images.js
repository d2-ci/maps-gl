...
const dataUri2image = dataUri => new Promise((resolve, reject) => {
...
const fetchSvg = url => fetch(url, {
  credentials
}).then(response => response.text()).then(svg => `data:image/svg+xml;charset=utf-8;base64,${window.btoa(svg)}`).then(dataUri2image);

// Load and add image to map
const loadImage = map => url => new Promise(resolve => {
  if (url.endsWith('.svg')) {
    fetchSvg(url).then(img => {
...
export const transformRequest = (url, resourceType) => resourceType === 'Image' ? {
  url,
  credentials
} : null;