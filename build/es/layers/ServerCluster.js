...
import { pointLayer, polygonLayer, outlineLayer, clusterLayer, clusterCountLayer } from '../utils/layers.js';
...
class ServerCluster extends Cluster {
  constructor(options) {
    ...
    // Meters per pixel
    _defineProperty(this, "getResolution", zoom => Math.PI * earthRadius * 2 / this.options.tileSize / Math.pow(2, zoom));
    ...
    _defineProperty(this, "onMoveEnd", () => {
      ...
      if (tiles.join('-') !== this.currentTiles.join('-')) {
        ...
        for (const tileId in this.tileClusters) {
          if (!tiles.includes(tileId) && Array.isArray(this.tileClusters[tileId])) {
    ...
    _defineProperty(this, "isOutsideBounds", bounds => ({
      geometry
    }) => {
      const {
        coordinates
